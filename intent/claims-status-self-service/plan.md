...
2. **Server skeleton with the safe defaults first** (`app/server.py`). Before any route exists, neutralise the base-class behaviours that would violate the spec on their own:
   ...
   - `_send_json` never answers in HTTP/0.9. (Added at PR review, 2026-09-02: the base class writes no status line and no headers when `request_version` is `HTTP/0.9`, which is the case for a genuine simple request and, on Python 3.12, for any request line it could not parse; 3.14 blanks the version before its own `400`, 3.12 does not, and CI runs 3.12. The helper sets `request_version` to `protocol_version` in that case so the R8 headers go out on every response.)
...
6. **Tests** (`tests/test_server.py`). A module-scoped fixture builds `create_server("127.0.0.1", 0)`, starts `serve_forever` on a daemon thread, yields the bound port, and calls `shutdown()` then `server_close()` on teardown. A small `request(method, path, body=None)` helper opens a fresh `http.client.HTTPConnection` per call and returns status, headers and decoded body. Tests, each named for what it proves:
   ...
   - `test_bad_request_line_is_json_and_unlogged`: open a raw socket and send `GET /claims/C-1001/status HTTP/1.1 extra\r\n\r\n` (four words, so the base class rejects it with its own `400`). Assert the response has a status line, the JSON content type and `Cache-Control: no-store`, and that neither the response nor `caplog.text` contains `C-1001`. This is the test for step 2.
   - `test_every_response_is_no_store` (parametrised, added at PR review 2026-09-02): one explicit assertion of `Cache-Control: no-store` for each status the routes produce: `200` (claim and `/health`), `404` (unknown id, malformed id, unknown path), `405`, `500`. The header was already asserted through `assert_json_headers` in the tests above; this names the R8 guard so its removal fails a test named for it.
   - `test_http_0_9_request_gets_status_line_and_headers` (added at PR review 2026-09-02): raw `GET /health\r\n\r\n` gets `HTTP/1.0 200`, `Cache-Control: no-store` and the JSON body rather than the base class's bare body. On 3.14 this is the only case that exercises the HTTP/0.9 guard in step 2; on 3.12 the bad-request-line test exercises it too.
...
**Python version mismatch between CI and the project.** `ci.yml` installs Python 3.12; `CLAUDE.md` says 3.14 and the local interpreter is 3.14.7. Everything this plan uses (`ThreadingHTTPServer`, `re`, `json`, `logging`) exists in 3.12, and the implementation must not use 3.14-only syntax. `ci.yml` is under a protected path, so aligning it is a separate change for the tech lead; this plan notes it and does not touch it. (Materialised on the first CI run, 2026-09-02: not syntax but behaviour. `http.server` in 3.12 answers an unparseable request line as HTTP/0.9, with no status line and no headers, where 3.14 sends a full response. Fixed in the code, see step 2, so the service behaves the same on both.)
...
- `make test` ends with `43 passed` (2 existing plus 15 new test functions, of which three are parametrised: 6 malformed-id cases, 8 methods x 2 paths = 16 method cases, and 7 no-store cases) and no `failed` or `error`. If the count differs, the test list above changed and this section is updated in the same commit. (Corrected at implementation from `15 passed`, which counted functions rather than pytest's expanded cases; raised from `35` at PR review when the two R8 tests were added.)
