...
- Run: `make run` (healthy output is three lines, `... INFO app.server letters endpoint disabled: CLAIMS_LETTERS_API_KEYS is not set` or `... letters endpoint enabled (N keys)`, then `... INFO app.server CLAIMS_BUILD is not set: /version reports unknown` or `... INFO app.server build identifier <id>`, then `... INFO app.server listening on 127.0.0.1:8000`, then one access line per request such as `GET /claims/{id}/status 200 0.1ms`; stop it with Ctrl-C). `CLAIMS_HOST` and `CLAIMS_PORT` override the bind address; the loopback default is deliberate, the service is meant to be reachable only from the portal or gateway. `CLAIMS_LETTERS_API_KEYS` (comma-separated, each at least 32 ASCII characters; an empty or repeated item is dropped with a `WARNING`) enables the letters route; unset, the route answers `404`, and a short or non-ASCII key stops startup with exit code 2 and an `ERROR` line naming the item's position, never the key. `CLAIMS_BUILD` is the build identifier `GET /version` reports (`{"build": "2026.09.03-7f3a1c2"}`); unset or empty, that route answers `{"build": "unknown"}`, which is not a usable build name so an operator can tell an unconfigured host apart. **Operator contract:** the value is echoed verbatim to any anonymous caller, so it must be a plain build identifier, never a secret, an internal hostname or a deploy path. Nothing validates it today; whether startup should reject a bad value is an open question for `intent/health-reports-version/spec.md`.
...
- `app/` is the whole service. `app/claims.py` holds the in-memory status table (`STATUSES`) and the single lookup, `get_status`. `app/letters.py` holds the letter-details fixture (`DETAILS`, same four ids) and `get_letter_details`, which returns exactly seven fields with `status` taken from `get_status`, plus `LettersUnavailable` for the future feed. `app/server.py` is the HTTP layer over both (standard-library `ThreadingHTTPServer`): `GET /claims/{id}/status`, `GET /claims/{id}/letter-details`, `GET /health`, `GET /version` (`{"build": "..."}` from `CLAIMS_BUILD`, anonymous like `/health`), `404` for everything else, `405` for non-GET on a known path. It authenticates only the letters route, by API key in `X-API-Key` (SHA-256 digests on the server object, constant-time compare, `401` before any id check, `404` on every method when no key is configured), and never logs or echoes a claim id, a letter field or a key; its only config is the four env vars above. There is no database.
